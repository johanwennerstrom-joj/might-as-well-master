...
class RealTidsInfo extends React.Component {
  ...
  realData = async props => {
    ...
    const api_call = await fetch(
      `${proxy}https://api.sl.se/api2/realtimedeparturesV4.${format}?key=${key}&siteid=${siteId}&timewindow=${timeWindow}`
    );
    ...
    const lineArray = lines.map(line => {
      if (line.TimeTabledDateTime !== line.ExpectedDateTime) {
        // Konvertera format timetable
        const times = [
          line.LineNumber,
          line.TimeTabledDateTime,
          line.ExpectedDateTime
        ];
